	...
	<div id="wrapper">
			...
			<!-- 신고 멤버 탭  -->
				<div class="w3-container reportMemberList">
					<div class="card-body" style="padding-bottom: 70px;">
						...
						<button id="memDisable" name="memDisable" class="btn btn-outline-danger">전체비활성화</button>
						</div>&nbsp;	
								...
								<tbody>
									<c:forEach items="${reportMemberList }" var="reportMember">
										...
										<tr>
											...
											<td>${reportMember.reportedName }</td>
											<td><span style="color:red;">
											${reportMember.reportCnt }</span></td>
											<td>                          
											<c:choose>
                                            <c:when test="${reportMember.mem_activation == 1 }">
		...
		<ul class="pagination" style="padding-left: 300px;">
			...
			<c:if test="${paging.curPage < 11 }">
				<!-- 첫페이지일때 -->
				<li class="disabled"><a href="/admin/reportMember"
			...
			<c:if test="${paging.curPage > 10 }">
				...
				<li><fmt:parseNumber var="p" integerOnly="true"
					...
					href="/admin/reportMember?curPage=${prevPage }" class="w3-button"
			...
			<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
				...
				<c:if test="${paging.curPage eq page }">
					<li class="active"><a href="/admin/reportMember?curPage=${page }"
						class="w3-button">${page }</a></li>
				</c:if>

				<c:if test="${paging.curPage ne page }">
					<li><a href="/admin/reportMember?curPage=${page }" class="w3-button">${page }</a>
					</li>
				</c:if>
			</c:forEach>
			...
			<c:if test="${paging.curPage eq paging.totalPage }">
				<!-- 마지막 페이지일때 -->
				<li class="disabled"><a
					href="/admin/reportMember?curPage=${paging.totalPage }" class="w3-button"
			...
			<c:if test="${paging.curPage ne paging.totalPage }">
				...
				<li><a href="/admin/reportMember?curPage=${nextPage }"
